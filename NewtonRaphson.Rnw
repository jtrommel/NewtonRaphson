...
\subsection{Estimating the probability $\pi$ of a binomial experiment}
...
The Likelihood for obtaining the registred results in all experiments is:
\begin{equation}
\begin{split}
P[X=x_{1}&=0|\pi]=(1-\pi) \\
P[X=x_{2}&=1|\pi]=\pi \\
...
\begin{equation}
\mathcal{L}(x_{1},x_{2}, \ldots x_{10})|\pi)=\prod_{i=1}^{i=10}P[X=x_{i}|\pi]=\pi^{8}(1-\pi)^{2}=\pi^{k}(1-\pi)^{n-k}
\end{equation}

The Log Likelihood function is:
\begin{equation}
\ell(x_{1},x_{2}, \ldots x_{n})|\pi)=k.ln(\pi) + (n-k).ln((1-\pi))
\end{equation}

It looks like this (Figure~\ref{fig:binom})
...
\begin{split}
\ell(x_{1},x_{2}, \ldots x_{n})|\pi) &=k.ln(\pi) + (n-k).ln(1-\pi) \\
\frac{d}{d\pi}\ell(x_{1},x_{2}, \ldots x_{n})|\pi) &=\frac{k}{\pi} - \frac{n-k}{1-\pi} \\
\frac{d^{2}}{d\pi^{2}}\ell(x_{1},x_{2}, \ldots x_{n})|\pi) &=-\frac{k}{\pi^{2}} -  \frac{n-k}{(1-\pi)^{2}}
\end{split}
\end{equation}

Again, the search for the $\pi$-value for which the Log Likelihood function reaches a maximum (we can see that the second derivative is always negative), has an analytical solution because $\frac{d}{d\pi}\ell(x_{1},x_{2}, \ldots x_{n})|\pi)=\frac{k}{\pi} -  \frac{n-k}{1-\pi}=0$ for $\pi=\frac{k}{n}$.
...
We find the correct result, but the iteration is much longer because the Taylor approximation in this case is truly an approximation!
